...
- Upstream renders a `SelectionIndicator` inside each `Tabs.Tab`; this port keeps
  that authoring shape (Kobalte has no selection-indicator primitive) and ports
  React Aria's shared-element transition so the indicator animates between tabs.
- `Tabs.ListContainer` accepts `initialShadow` to server-render the overflow
  chevron and fade for lists known to overflow, and `hideScrollButtons` to
  omit the chevron buttons entirely.
- The selected tab is kept in view: scrolled to instantly on mount, with an
  animated scroll on selection change.
